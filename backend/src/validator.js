...
function cycleHasAllowedException(cycleNodePath, edges, nodesById) {
  ...
  const cycleEdges = edges.filter(
    (e) => cycleNodeSet.has(e.source) && cycleNodeSet.has(e.target),
  )
...
export function validatePipelineGraph(payload) {
  ...
  for (const edge of edges) {
    ...
    const compatible = sourceSpec.outputs.some((outType) =>
      targetSpec.inputs.includes(outType),
    )
  ...
  for (const cycle of cycles) {
    if (!cycleHasAllowedException(cycle, edges, nodesById)) {
      ...
      const cycleEdgeIds = edges
        .filter(
          (edge) =>
            cycleNodeSet.includes(edge.source) && cycleNodeSet.includes(edge.target),
        )
        .map((edge) => edge.id)
